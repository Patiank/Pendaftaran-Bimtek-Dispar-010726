...
export const generateCertificateImage = (data: CertificateData): Promise<string> => {
  return new Promise(async (resolve, reject) => {
    ...
    const drawThreeDynamicTexts = (targetCtx: CanvasRenderingContext2D, width: number, height: number) => {
      ...
      // Set text alignment to center as requested by user
      targetCtx.textAlign = "center";
      ...
      // Center alignment requested by user
      targetCtx.textAlign = "center";
    ...
    try {
      ...
      ctx.lineWidth = 3;
      ctx.beginPath();
      ctx.moveTo(120, headerY + 105);
      ctx.lineTo(w - 120, headerY + 105);
      ctx.stroke();

      ctx.strokeStyle = "#0d5c3a";
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(120, headerY + 110);
      ctx.lineTo(w - 120, headerY + 110);
      ctx.stroke();
